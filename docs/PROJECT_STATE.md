...
- 权威工作区：本仓库根目录；Beta 2 源码与下一代开发说明已于 2026-09-10
  ...
  恢复的 Block 草稿继续编辑及中断写入恢复仍未完成。旧历史审计
  ...
  已符合 V1 范围，无需再次改名。最新 GUI 增量已获本轮单次源码提交/推送授权，
  同步回执将在远端核验后记录。上次原生观察为
  Mac 锁定，待手动解锁后验收。
...
- M4 核心 API 可以显式采集选定文件的原始字节，记录相对路径/摘要，并将调用方
  ...
  草稿独立保留，不自动应用、切换项目或编译；Block JSON 尚不能直接恢复为编辑会话。
  这不是自动崩溃恢复或完整 M4；详情和平台/断电限制见
  `docs/v1-m4-checkpoint-gui-verification-2026-09-11.md`。
...
| 对象 | 状态 | 说明 |
| --- | --- | --- |
| 当前源码 | Verified V1 checkpoint GUI; synchronization authorized | 2.1.0-beta.2；前批 `c521041` 已推送至 `codex/v1-development`，最新 M4 GUI 增量全套通过并获单次同步授权，待远端核验；保留 Beta 远端 `release/2.1`；不是完整 V1 验收或发布 |
...
- V1 已完成普通源码和 Block 的 M1 只读切片与实际 FINAL 输入/日志绑定；M2 已有
  ...
  字节恢复/重开，GUI 选定清单、实际源/Block 草稿捕获与恢复审阅也已验证；恢复草稿
  继续编辑、中断 Block 写入恢复和迁移仍待实现/验收。
...
- M4 旧文本历史的路径/摘要/确认归属修复已通过有界合成验证，未触及真实用户文件。
  ...
  新项目检查点已有 GUI 捕获/审阅工作流，但不把它或文本历史修复标记为完整 M4。
