...
class ModalWorkflowKeyboardTests(TestCase):
    ...
    def test_modal_comboboxes_remain_reachable_with_text_list_tab_policy(self):
        ...
        try:
            ...
            wizard.advanced_toggle.setChecked(True)
            ...
            for dialog, before, first, second in (
                    (wizard, wizard.language_combo, wizard.template_combo, wizard.parent_edit),
            ...
            try:
                ...
                for scale in (0.9, 1.0, 1.1, 1.25, 1.5):
                    with self.subTest(scale=scale):
                        ...
                        for _ in range(5):
                            application.processEvents()
                        self.assertIs(application.focusWidget(), panel.jump_button)
                        self.assertTrue(panel.jump_button.visibleRegion().contains(panel.jump_button.rect()))
                        QTest.keyClick(panel.jump_button, Qt.Key.Key_Tab)
                        for _ in range(5):
                            application.processEvents()
                        ...
                        for _ in range(5):
                            application.processEvents()
                        self.assertIs(application.focusWidget(), panel.jump_button)
                        QTest.keyClick(panel.jump_button, Qt.Key.Key_Backtab)
                        for _ in range(5):
                            application.processEvents()
                        self.assertIs(application.focusWidget(), panel.refresh_button)
                        self.assertTrue(panel.refresh_button.visibleRegion().contains(panel.refresh_button.rect()))
