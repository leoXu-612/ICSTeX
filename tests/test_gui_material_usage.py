...
class MaterialUsageGuiTests(TestCase):
    ...
    def test_worker_failure_without_report_stops_monitoring(self):
        with self.assertLogs("app.gui.material_usage_controller", level="ERROR") as logs, \
             patch("app.gui.material_usage_controller.check_materials", side_effect=ValueError("private fixture text")):
            self.window.materials.request()
            self.wait_for(lambda: not self.window.materials.is_busy)
        self.assertIsNone(self.window.images_panel.material_report)
        self.assertIn("结果未知", self.window.images_panel.check_status.text())
        self.assertFalse(self.window.materials._timer.isActive())
    ...
    def test_monitoring_survives_failed_old_owner_and_pending_handoff(self):
        controller = self.window.materials
        def launch(request):
        ...
        with patch.object(controller, "_launch", side_effect=launch):
            ...
            controller._finished(first, check_materials(first), "")
            self.assertIs(controller._active, pending)
            self.assertIsNone(controller._report)
            controller._finished(pending, check_materials(pending), "")
        self.assertTrue(controller._timer.isActive())
        self.assertTrue(controller._baselines)
        baselines = dict(controller._baselines)
        with patch.object(self.window.dependencies, "generation_for", return_value=controller._key()[-1] + 1):
            controller.reconcile()
        self.assertIsNone(controller._report)
        self.assertEqual(controller._baselines, baselines)
        self.assertFalse(controller._timer.isActive())

    def test_edit_undo_rejects_worker_even_when_editor_signals_are_blocked(self):
        controller = self.window.materials
        def launch(request):
            controller._active, controller._cancelled = request, threading.Event()
        with patch.object(controller, "_launch", side_effect=launch):
            controller.request()
            request = controller._active
            report = check_materials(request)
            editor = self.window.current_tab().editor
            ...
            controller._finished(request, report, "")
        self.assertIsNone(controller._report)
        self.assertFalse(controller._timer.isActive())
