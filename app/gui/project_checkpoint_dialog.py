...
def checkpoint_close_guard(window):
    archive = getattr(QApplication.instance(), "_icstex_project_archive_dialog", None)
    if archive is not None and isValid(archive) and (archive.window is window
            or archive.lease is not None and window in archive.lease.windows):
        archive.reject()
        if archive.busy:
            return False
