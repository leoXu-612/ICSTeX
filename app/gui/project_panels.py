...
class ProjectWizardDialog(QDialog):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.language_combo = DialogComboBox()
        self.language_combo.addItems(["全部语言", *dict.fromkeys(t.language for t in self._templates.values())])
        self.engine_combo = DialogComboBox()
        for engine in LaTeXEngine:
            self.engine_combo.addItem("自动选择（推荐）" if engine is LaTeXEngine.AUTO else engine.display_name, engine.value)
        ...
        self.source_toggle = QPushButton("查看模板源码")
        self.source_toggle.setCheckable(True)
        self.source_toggle.setAutoDefault(False)
        self.source_toggle.toggled.connect(self.preview.setVisible)
        self.preview.hide()
        self.layout_preview = QLabel("选择模板后查看排版示例")
        self.layout_preview.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.layout_preview.setWordWrap(True)
        self.layout_preview.setMinimumHeight(180)
        self.layout_preview.setAccessibleName("模板排版示例（非当前项目编译结果）")
        self.advanced_toggle = QPushButton("高级设置")
        self.advanced_toggle.setCheckable(True)
        self.advanced_toggle.setAutoDefault(False)
        try:
            self._preview_hashes = json.loads(asset_path("template-previews/manifest.json").read_text(encoding="utf-8"))
    ...
    def _filter_templates(self) -> None:
        ...
        self.template_combo.addItem("请选择要写的内容…", None)
        for template in self._templates.values():
            if language == "全部语言" or template.language == language:
                self.template_combo.addItem(f"{template.title} · {template.language}", template.key)
        self.template_combo.setCurrentIndex(max(0, self.template_combo.findData(previous)))
    ...
    def values(self) -> ProjectInitSpec:
        ...
        if key not in self._templates:
            raise ValueError("请先选择要写的内容，再创建项目。")
    ...
    def selected_engine(self) -> LaTeXEngine:
    ...
    def _update_template(self) -> None:
        """Template selection owns source/hash/image work; engine changes do not."""
        template = self._templates.get(str(self.template_combo.currentData()))
        self.buttons.button(QDialogButtonBox.StandardButton.Ok).setEnabled(template is not None)
        self.layout_preview.clear()
        if template is None:
            self.preview.clear()
            self._update_engine_hint()
            self.layout_preview.setText("选择模板后查看排版示例")
            return
        self.preview.setPlainText(template.text)
        preview_file = asset_path(f"template-previews/{template.key}.png")
        digest = hashlib.sha256(template.text.encode("utf-8")).hexdigest()
        if self._preview_hashes.get(template.key) == digest:
            pixmap = QPixmap(str(preview_file))
            if not pixmap.isNull():
                ratio = self.devicePixelRatioF()
                pixmap = pixmap.scaled(round(310 * ratio), round(210 * ratio), Qt.AspectRatioMode.KeepAspectRatio,
                                      Qt.TransformationMode.SmoothTransformation)
                pixmap.setDevicePixelRatio(ratio)
                self.layout_preview.setPixmap(pixmap)
        if self.layout_preview.pixmap().isNull():
            self.layout_preview.setText("此模板暂无排版示例，可展开查看源码。\n创建后主动编译即可查看实际 PDF。")
        self._update_engine_hint()

    def _update_engine_hint(self) -> None:
        """Update availability copy without resetting preview selection or scroll."""
        template = self._templates.get(str(self.template_combo.currentData()))
        if template is None:
            self.template_hint.setText("先选择写作语言与模板。没有 LaTeX 环境也可以创建和编辑。")
            return
    ...
    def accept(self) -> None:
        try:
            ...
            message = ("保存位置不存在，请选择已有文件夹。" if isinstance(exc, FileNotFoundError) else
                       "无法写入保存位置，请选择有写入权限的文件夹。" if isinstance(exc, PermissionError) else
                       str(exc))
            self.error.setText(f"未创建项目：{message}\n请检查保存位置或更换项目名称；当前输入已保留。")
            return
        super().accept()

    def _build(self) -> None:
        layout = QVBoxLayout(self)
        intro = QLabel("选择要写的内容，创建后修改标题，再点击“正式编译”查看 PDF。")
        intro.setWordWrap(True)
        layout.addWidget(intro)
        body = QWidget()
        form = QFormLayout(body)
        form.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapLongRows)
        form.addRow("写作语言", self.language_combo)
        form.addRow("我要写", self.template_combo)
        form.addRow(self.template_hint)
        form.addRow("保存位置", self._folder_row())
        form.addRow("项目名称", self.name_edit)
        form.addRow(self.destination)
        form.addRow(self.open_new_window)
        form.addRow("首页示例", self.layout_preview)
        note = QLabel("模板首页的静态示例（非当前项目 PDF），创建后请主动编译。")
        note.setWordWrap(True)
        form.addRow(note)
        form.addRow(self.source_toggle)
        ...
        advanced_form.addRow("编译器", self.engine_combo)
        advanced_form.addRow(self.profile_enabled)
        form.addRow(self.advanced)
        self.advanced.hide()
        self.advanced_toggle.toggled.connect(self.advanced.setVisible)
        scroll = scrollable_panel(body)
        scroll.setObjectName("projectWizardScroll")
        scroll.followReadOnlyText(self.preview)
    ...
    def _browse_parent(self) -> None:
        folder = QFileDialog.getExistingDirectory(self, "选择保存位置", self.parent_edit.text())
        if folder:
