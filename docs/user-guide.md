...
> 适用版本：未发布开发源码，性能/UI 优化仍在验收中；公开安装版可能不同。
...
1. 新建：工具栏“新建项目”选择项目与模板；临时源码可用“文件 → 新建文档”；
2. 写入内容：源码区输入 LaTeX，或“编辑 → 编辑公式”插入公式；
3. 编译：先点“正式编译”授权当前项目的首次构建；自动编译开启后，后续输入暂停并保存后按预览设置更新；
4. 预览：右侧 PDF 面板即时刷新；双击 PDF 可跳回源码（SyncTeX）；
5. 导出：当前开发源码从“文件 → 准备提交”单独保存、正式编译并审阅后交付；“导出 PDF”进入同一流程。公开安装版尚未包含此流程。
